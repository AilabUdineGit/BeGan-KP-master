...
def load_vocab(opt):
    ...
    # opt.vocab = "/Users/r17935avinash/Desktop/keyphrase-generation-rl/data/kp20k_seperated"
    ...
    # print("The total size is",)
    return word2idx, idx2word, vocab


def extract_data_subset(dataset, random_extract=False, sample_size=-1):
    """
    ...
    if random_extract:
        ...
        #     random.seed(seed)
        indices = random.sample(list(range(len(dataset))), sample_size)
    else:
        indices = list(range(len(dataset[:sample_size-1])))

    return SubsetRandomSampler(indices)


def load_sampled_data_and_vocab(opt, load_train=True, sampled=False):
    # load vocab
    # print(opt)
    ...
    # opt.data = "/Users/r17935avinash/Desktop/keyphrase-generation-rl/data/kp20k_seperated"
    if load_train:  # load training dataset
        if not opt.one2many:  # load one2one dataset
            ...
            train_one2many_dataset = KeyphraseDataset(train_one2many, word2idx=word2idx, idx2word=idx2word, type='one2many',
                                                      ...
                                                      remove_src_eos=opt.remove_src_eos, title_guided=opt.title_guided)
            if sampled:
                train_sampler = extract_data_subset(train_one2many_dataset,
                                                    random_extract=True,
                                                    sample_size=opt.sample_size)  # gl: test
                train_loader = DataLoader(dataset=train_one2many_dataset,
                                          collate_fn=train_one2many_dataset.collate_fn_one2many,
                                          num_workers=opt.batch_workers, batch_size=opt.batch_size, pin_memory=True,
                                          sampler=train_sampler)  # gl: test
            else:
                train_loader = DataLoader(dataset=train_one2many_dataset,
                                          collate_fn=train_one2many_dataset.collate_fn_one2many,
                                          num_workers=opt.batch_workers, batch_size=opt.batch_size, pin_memory=True,
            ...
            valid_one2many_dataset = KeyphraseDataset(valid_one2many, word2idx=word2idx, idx2word=idx2word, type='one2many',
                                                      ...
                                                      remove_src_eos=opt.remove_src_eos, title_guided=opt.title_guided)
            if sampled:
                valid_sampler = extract_data_subset(valid_one2many_dataset,
                ...
                valid_loader = DataLoader(dataset=valid_one2many_dataset,
                                          ...
                                          num_workers=opt.batch_workers, batch_size=opt.bert_validation_batch_size, pin_memory=True,
                                          sampler=valid_sampler)  # gl: test
            else:
                valid_loader = DataLoader(dataset=valid_one2many_dataset,
                                          collate_fn=valid_one2many_dataset.collate_fn_one2many,
                                          num_workers=opt.batch_workers, batch_size=opt.bert_validation_batch_size, pin_memory=True,
        ...
        test_one2many_dataset = KeyphraseDataset(test_one2many, word2idx=word2idx, idx2word=idx2word, type='one2many',
                                                 delimiter_type=opt.delimiter_type, load_train=load_train,
                                                 remove_src_eos=opt.remove_src_eos, title_guided=opt.title_guided)
        # test_sampler = extract_data_subset(test_one2many_dataset,
        #                                     random_extract=False,
        #                                     sample_size=opt.sample_size)  # gl: test
