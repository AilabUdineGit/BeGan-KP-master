...
class SequenceGenerator(object):
    ...
    def sample(self, src, src_lens, src_oov, src_mask, oov_lists, max_sample_length, greedy=False, one2many=False, one2many_mode=1, num_predictions=1, perturb_std=0, entropy_regularize=False, title=None, title_lens=None, title_mask=None):
        ...
        if self.model.separate_present_absent:  # gl: False
        ...
        # print('1')  # gl
        # print(unfinished_mask)  # gl
        unfinished_mask_all = [unfinished_mask]
        # print('2')  # gl
        # print(unfinished_mask_all)  # gl
        ...
        if entropy_regularize:  # gl: False
            entropy = torch.zeros(batch_size).to(src.device)
        else:
            entropy = None

        for t in range(max_sample_length):  # gl: max_sample_length=6
            if t > 0:
                ...
                unfinished_mask = unfinished_mask.type(torch.uint8).to(src.device)  # gl: necessario sennò si rompe
                # print('unfinished_mask_1 ' + str(unfinished_mask))  # gl
                unfinished_mask = unfinished_mask.unsqueeze(1)
                # print('unfinished_mask_2 ' + str(unfinished_mask))  # gl
            ...
            if t == 0:
                h_t = h_t_init
                y_t = y_t_init
            elif one2many and one2many_mode == 2 and re_init_indicators.sum().item() > 0:  # False (one2many_mode=1)
                h_t = []
                y_t = []
            ...
            elif one2many and one2many_mode == 3 and re_init_indicators.sum().item() > 0:  # False (one2many_mode=1)
                h_t = h_t_next
                y_t = []
                for batch_idx, (indicator, pred_count) in enumerate(zip(re_init_indicators, pred_counters)):
                    if indicator.item() == 1 and pred_count.item() < num_predictions:
                        # some examples complete one keyphrase
                        # reset input to <BOS>
                        y_t.append(y_t_init[batch_idx].unsqueeze(0))
            ...
            if self.model.use_target_encoder:  # False
                # encode the previous token using target encoder
            ...
            if self.model.separate_present_absent:  # False
                # update the is_absent vector
                for i in range(batch_size):
            ...
            # gl: qui sotto si creano le possibili KPs partendo dalle prime t parole estratte in precedenza
        ...
        # print('tipo log_selected_token_dist: ' + str(log_selected_token_dist.type()))  # gl
        # print(unfinished_mask_all)  # gl
