...
class RNNDecoder(nn.Module):
    ...
    def forward(self, y, h, memory_bank, src_mask, max_num_oovs, src_oov, coverage, decoder_memory_bank=None,
                target_encoder_state=None, goal_vector=None):
        """
        :param y: [batch_size]
        ...
        if self.use_target_encoder:
            ...
            rnn_input = torch.cat([rnn_input, target_encoder_state.detach()],
                                  dim=2)  # [1, batch_size, embed_size+target_encoder_size]
        ...
        if self.review_attn:
            ...
            review_context, review_attn_dist, _ = self.review_attention_layer(last_layer_h_next, decoder_memory_bank,
                                                                              src_mask=None, coverage=None)
            ...
            vocab_dist_input = torch.cat((context, last_layer_h_next, review_context),
                                         dim=1)  # [B, memory_bank_size + decoder_size + decoder_size]
        else:
        ...
        if self.copy_attn:
            if self.goal_vector_mode == 2:
                p_gen_input = torch.cat((context, last_layer_h_next, y_emb.squeeze(0), goal_vector.squeeze(0)),
                                        dim=1)  # [B, memory_bank_size + decoder_size + embed_size + goal_vector]
            else:
    ...
    def forward_bah(self, y, h, memory_bank, src_mask, max_num_oovs, src_oov, coverage):
        ...
        # pass the concatenation of the input embedding and context vector to the RNN
        # insert one dimension to the context tensor
        rnn_input = torch.cat((y_emb, context.unsqueeze(0)),
                              2)  # [1, batch_size, embed_size + num_directions * encoder_size]
...
if __name__ == '__main__':
    ...
    pad_idx = 0
    review_attn = True
    decoder = RNNDecoder(vocab_size, embed_size, decoder_size, num_layers, memory_bank_size, coverage_attn, copy_attn,
                         review_attn, pad_idx, dropout)
    batch_size = 5
    max_src_seq_len = 7
    ...
    final_dist, h_next, context, attn_dist, p_gen, coverage = decoder(y, h, memory_bank, src_mask, max_num_oovs,
                                                                      input_seq_oov, coverage, trg_side_memory_bank)
    print("Pass")
